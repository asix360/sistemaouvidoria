...
export const Header: React.FC<HeaderProps> = ({ setActiveTab, onOpenCitizenPortal, onToggleMobileMenu }) => {
  const {
    theme,
    themeMode,
    setTheme,
    ...
    <header className="h-16 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 px-4 sm:px-6 flex items-center justify-between sticky top-0 z-30 shadow-xs">
      ...
      <div className="flex items-center gap-2 sm:gap-3 flex-1 max-w-xl">
        ...
        <button
          ...
          className="p-2 rounded-xl text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors md:hidden shrink-0"
        ...
        <UpaLogo size="sm" showSubtitle={false} className="shrink-0 hidden sm:flex" />
